...
if(typeof document.body.style.maxWidth==="undefined"&&options.maxwidth){var widthSupport=function(){$this.css("width","100%");if($this.width()>maxw){$this.css("width",maxw)}};widthSupport();$(window).bind("resize",function(){widthSupport()})}})}})(jQuery,this,0);(function(){'use strict';function scaleImageMap(){function resizeMap(){function resizeAreaTag(cachedAreaCoords,idx){function scale(coord){var dimension=(1===(isWidth=1-isWidth)?'width':'height');return padding[dimension]+Math.floor(Number(coord)*scalingFactor[dimension])}
...
if('jQuery' in window){jQuery.fn.imageMapResize=function $imageMapResizeF(){return this.filter('map').each(scaleImageMap).end()}}})();$(document).ready(function(){$("#Tbilisi").mousemove(function(pos){$(".map__tbilisi").show();$(".map__tbilisi").css('left',(pos.pageX+20)+'px').css('top',(pos.pageY+20)+'px')}).mouseleave(function(){$(".map__tbilisi").hide()});$("#Rustavi").mousemove(function(pos){$(".map__rustavi").show();$(".map__rustavi").css('left',(pos.pageX+20)+'px').css('top',(pos.pageY+20)+'px')}).mouseleave(function(){$(".map__rustavi").hide()});$("#Marneuli").mousemove(function(pos){$(".map__marneuli").show();$(".map__marneuli").css('left',(pos.pageX+20)+'px').css('top',(pos.pageY+20)+'px')}).mouseleave(function(){$(".map__marneuli").hide()});$('map').imageMapResize()});$(document).ready(function(){var width=$(window).width();if(width<=360){languages=$('.languages').detach();$('.navigation').append(languages)}});$(function(){var width=$(window).width();pull=$('.header__toggle');menu=$('.navigation__menu');languages=$('.languages');menuHeight=menu.height();$(pull).on('click',function(e){e.preventDefault();menu.slideToggle();menu.css('display','flex');if(languages.is(':hidden')){languages.css('display','block')}else{languages.css('display','none')}});$(window).resize(function(){if(width>320&&menu.is(':hidden')){menu.removeAttr('style')}
if(width>360){languages=$('.languages').detach();$('.header__toggle').before(languages);$('.languages').css('display','block')}})});$(document).ready(function(){var height=$('.header').outerHeight()+15;$('.header-invisible-clone').css('height',height+'px')});$(window).resize(function(){var height=$('.header').outerHeight()+15;$('.header-invisible-clone').css('height',height+'px')});$(document).ready(function(){$('.whoweare__images').click(function(){$('.whoweare__images').removeClass('whoweare__images--active');$(this).toggleClass('whoweare__images--active')})})